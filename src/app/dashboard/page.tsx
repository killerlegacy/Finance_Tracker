...
import {
  Wallet, Plus, LayoutDashboard, Receipt, CreditCard, Target, Repeat, User, LogOut,
} from 'lucide-react';
import { AnyRecord, Transaction, Account, Budget, Subscription, Profile, TabType } from '../lib/types';
import { getSession, logout, getUserDataKey } from '../lib/auth';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const dataKey = getUserDataKey(session.user_id);
    const currencyKey = `ft_currency_${session.user_id}`;
    ...
    try {
      const stored = localStorage.getItem(dataKey);
      if (stored) {
        const data: AnyRecord[] = JSON.parse(stored);
        const profile = data.find((d) => d.type === 'profile') as Profile | undefined;
        if (!profile?.onboarding_complete) {
          router.replace('/onboarding');
          return;
        }
        // Use currency from profile if no separate key
        if (!localStorage.getItem(currencyKey) && profile?.currency) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(getUserDataKey(userId), JSON.stringify(allData));
    } catch {}
  }, [allData, hydrated, userId]);
  ...
  const addRecord = (record: AnyRecord) => setAllData((prev) => [...prev, record]);
    ...
    <div className="h-full w-full flex flex-col overflow-hidden">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 shrink-0 overflow-x-auto">
        <div className="flex gap-1 min-w-max">
          {tabs.map((tab) => (
            <button
              key={tab.key}
              onClick={() => { setCurrentTab(tab.key); setShowUserMenu(false); }}
      ...
      <main className="flex-1 overflow-auto p-4" onClick={() => setShowUserMenu(false)}>
        ...
        {currentTab === 'transactions' && (
          <TransactionsView transactions={transactions} currency={currency} onDelete={requestDelete} />
        )}
        {currentTab === 'accounts' && (
          <AccountsView accounts={accounts} currency={currency} onDelete={requestDelete} onAdd={() => setShowAccount(true)} />
        )}
        {currentTab === 'budgets' && (
          <BudgetsView budgets={budgets} transactions={transactions} currency={currency} onDelete={requestDelete} onAdd={() => setShowBudget(true)} />
        )}
        {currentTab === 'subscriptions' && (
          <SubscriptionsView subscriptions={subscriptions} currency={currency} onDelete={requestDelete} onAdd={() => setShowSubscription(true)} />
        )}
        {currentTab === 'profile' && (
          <ProfileView
            profile={profile}
      ...
      {/* Modals */}
      <AddTransactionModal isOpen={showAddTx} onClose={() => setShowAddTx(false)} onAdd={(tx) => { addRecord(tx); showToast('Transaction added!'); }} currency={currency} />
      <AccountModal isOpen={showAccount} onClose={() => setShowAccount(false)} onAdd={(acc) => { addRecord(acc); showToast('Account added!'); }} />
      <BudgetModal isOpen={showBudget} onClose={() => setShowBudget(false)} onAdd={(b) => { addRecord(b); showToast('Budget set!'); }} existingCategories={budgets.map((b) => b.category)} />
      <SubscriptionModal isOpen={showSubscription} onClose={() => setShowSubscription(false)} onAdd={(s) => { addRecord(s); showToast('Subscription added!'); }} />
      <ProfileModal isOpen={showProfile} onClose={() => setShowProfile(false)} onSave={(p) => { upsertProfile(p); showToast('Profile saved!'); }} existingProfile={profile} />
